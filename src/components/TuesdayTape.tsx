...
export default function TuesdayTape() {
  ...
  async function submit(e: FormEvent) {
    ...
    if (file && file.size > MAX_BYTES) {
      setError("That file is over 400MB. Upload it to YouTube (unlisted) or Drive and paste the link instead.");
      return;
    }
    setBusy(true);
    setError(null);
      ...
      {canPost && (
        <form onSubmit={submit} className="panel grid gap-3 p-4 sm:grid-cols-2">
          ...
          <label className={`${input} cursor-pointer text-cream-dim`}>
            {file ? `${file.name} (${(file.size / 1048576).toFixed(1)} MB)` : "…or upload a file under 400MB"}
