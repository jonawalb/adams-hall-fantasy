...
export default function RivalriesView({ scope, current }: { scope: Scope; current: string }) {
  const seasons = scopedSeasons(availableSeasons().map((y) => loadSeason(y)!), scope);
  ...
  const kicker = { all: "All games", regular: "Regular season only", playoffs: "Postseason weeks, consolation included" }[scope];
  const title = { all: "RIVALRIES", regular: "REGULAR SEASON RIVALRIES", playoffs: "PLAYOFF RIVALRIES" }[scope];
  ...
  return (
    <div className="space-y-8">
      <header className="rise space-y-4">
        <div>
          <p className="kicker">Head-to-Head · {kicker} · 2021–present</p>
          ...
          <p className="mt-2 max-w-2xl text-sm text-cream-dim">
            Read across: the row owner&rsquo;s record against each column.
          </p>
        </div>
