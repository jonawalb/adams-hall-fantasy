...
export default function TuesdayRecap() {
  ...
  async function submit(e: FormEvent) {
    ...
    if (file && file.size > MAX_BYTES) {
      setError("File is over 50MB. Compress the audio or upload to a hosting service and paste the link.");
      return;
    }
    setBusy(true);
    setError(null);
      ...
      {canPost && (
        <form onSubmit={submit} className="panel grid gap-3 p-4 sm:grid-cols-2">
          ...
          <label className={`${input} cursor-pointer text-cream-dim`}>
            {file ? `${file.name} (${(file.size / 1048576).toFixed(1)} MB)` : "…or upload audio/video under 50MB"}
